            ...
            {products.map((product) => (
              <div key={product.id} className="bg-white p-5 rounded-lg shadow border border-gray-200 flex flex-col justify-between">
                <div>
                  ...
                  <h2 className="text-xl font-semibold mt-3">{product.nama}</h2>
...
function CartModal({ cart, setCart, rentalSchedule, setRentalSchedule, onClose }) {
  ...
  // State untuk menyimpan pilihan item dan qty khusus pasang & bongkar
  ...
  const handleProceedToForm = () => {
    ...
    // Inisialisasi default item pasang & bongkar jika belum ada
  ...
  const handleCheckout = async (e) => {
    ...
    try {
      // Filter rincian pasang bongkar yang dipilih saja
      ...
      const bookingPayloads = cart.map((item) => ({
        ...
        total_biaya: item.harga * item.qty,
        status: 'pending'
      }))
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white p-6 rounded-lg max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        {/* TAMPILAN 1: KERANJANG & JADWAL SEWA/PENGECEKAN */}
        {step === 'cart' && (
          <div>
        ...
        {/* TAMPILAN 2: ISI DATA PEMESANAN & KONFIRMASI */}
        {step === 'form' && (
          <div>
            <div className="flex justify-between items-center mb-4">
            ...
            <form onSubmit={handleCheckout} className="space-y-3">
              ...
              {/* FIELD KONDISIONAL ANTAR JEMPUT */}
              ...
              {/* FIELD KONDISIONAL PASANG & BONGKAR */}
